...
vits_port = os.getenv("TTS_VITS_PORT", "5941")
rvc_port = os.getenv("TTS_RVC_PORT", "5951")
tts_port = os.getenv("TTS_GPU_PORT", "5010")
blips_port = os.getenv("TTS_BLIPS_PORT", "5020")

max_active_subrequests = int(os.getenv("TTS_MAX_SUBREQUESTS", 5))

slow_log_secs = float(os.getenv("TTS_SLOW_LOG_SECONDS", -1))
...
def schedule_watchdog():
	...
	if watchdog:
		watchdog_timer = threading.Timer(0.2, ping_watchdog)
...
def text_to_speech_handler(voice, text, filter_complex, pitch, authed, force_regenerate, stats, special_filters):
	...
	hashtext = f"#v11#tts#{voice.lower()}#{clean_text}#{filter_complex}#{pitch}#{json.dumps(special_filters)}#{random.randint(0, rand_cap)}#{len(clean_text)}#"
	hash = hashlib.sha224(hashtext.encode()).hexdigest().lower()
	
	identifier = f"tts-{hash}"
	
	log_prefix = f"tts-{hash[0:7]}"
	path_prefix = "./cache"
	if file_store:
		path_prefix = os.path.expanduser(file_store)

	subpath = f"v11/tts/{hash[0:2]}/{hash[2:4]}/{hash[4:6]}/"
	...
	if (file_store) and (not force_regenerate) and (random.randint(0, 9) != 0 or (not authed)):
		...
		if os.path.isfile(f"{path}.ogg.gz") and os.path.isfile(f"{path}.len.txt.gz"):
	...
	with ffmpeg_open(ffmpeg_args) as ffmpeg_proc:
		...
		#do the requests (us -> vits -> us -> rvc -> us)
		with FuturesSession(max_workers = max_active_subrequests) as vits_session:
			request_futures = OrderedDict()
			for i, sentence in enumerate(segmenter.segment(clean_text)):
				if len(''.join(ch for ch in sentence if ch not in string.punctuation)) < 1:
					continue
				steptime = time.time()
				request_future = vits_session.get(
					f"http://localhost:{vits_port}/generate-vits?format=numpy", 
					json={ 'text': sentence, 'voice': voice },
					headers={
						'Accept': 'application/octet-stream',
						'X-Log-Prefix': f"{log_prefix}-{i}",
					}
				)
				request_future.subrequest_index = i
				request_futures[request_future] = None

			
			steptime = time.time()
			with FuturesSession(max_workers = max_active_subrequests) as rvc_session:
				for request_future in as_completed(request_futures):
					response = request_future.result()
					request_futures[request_future] = response
					request_future.rvc_subrequest = rvc_session.get(
						f"http://localhost:{rvc_port}/generate-rvc?pitch={int(pitch)}&voice={url_quote(voice)}",
						data=response.content,
						headers={
							'Content-Type': 'application/octet-stream',
							'Accept': 'audio/wav',
							'X-Log-Prefix': f"{log_prefix}-{request_future.subrequest_index}",
						}
					)
				vits_time = time.time() - steptime

				steptime = time.time()
				responses = []
				for request_future in request_futures:
					result = request_future.rvc_subrequest.result()
					responses.append(result)
				rvc_time = time.time() - steptime
				print(f"{log_prefix} {vits_time=}, {rvc_time=}")
			...
			for response in responses:
				...
				try:
					...
					print(f"{log_prefix}  pydub error: {repr(e)}.")
					print(f"{log_prefix}  {hashtext=} {response.status_code=} {json.dumps(dict(response.headers))=}.")
					print(f"{log_prefix}  {len(response.content)=}.")
		...
		with io.BytesIO() as data_bytes:
			...
			print(f"{log_prefix} {ffmpeg_time=} {len(ffmpeg_stdout)=} \n {ffmpeg_metadata_output=}", file=sys.stderr)
	...
	stats['avg_request_time'] = mc_avg(stats['avg_request_time'], request_time)
	if slow_log_secs > 0 and request_time > slow_log_secs:
		message = f"{log_prefix} slowlog: {request_time=}({vits_time=},{rvc_time=},{ffmpeg_time=})\n\t{text=} {hashtext=} {ffmpeg_args=} {ffmpeg_metadata_output=}\n"
		print(message)
		with open('tts_slowlog.txt', 'a') as f:
			f.write(message)
	return response
...
stop_point = 65536*(random.randint(2,4)*random.random())
@app.route("/health-check")
def tts_health_check():
	global req_count, cache_hits, cache_misses
	gc.collect()
	if req_count > int(stop_point / max(min(avg_request_delay, 5), 0.1)):
		watchdog.notify_error()
