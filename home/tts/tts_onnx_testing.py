...
class OnnxRVCInferencer():
	def loadModel(self, file: str):
		...
		first_input_type = self.onnx_sess.get_inputs()[0].shape
...
class FairseqHubertOnnx(Embedder):
	def loadModel(self, file: str, dev: device = "cuda:0", isHalf: bool = True) -> Embedder:
		...
		self.onnx_sess = ort.InferenceSession(
			file,
			sess_options=tg_tts_sess_options,
			providers=tg_tts_tensor_providers,
		)
...
with torch.inference_mode():
	...
	encoder_2.loadModel("hubert_base_simple.shaped.onnx")
	print("Loaded")
	time.sleep(10)
	...
	print("Loaded")
	time.sleep(10)
	print("Loading Vits model stage 2")
	#vits.export_onnx("tgstation_onnx.onnx")
	vits.load_onnx2("tgstation_onnx.simplified.shaped.onnx", True)
	print("Loaded")
	time.sleep(10)
### READ ME
...
os.makedirs('samples/v2', exist_ok=True)
...
def text_to_speech_blips():
	...
	with io.BytesIO() as data_bytes:
		with torch.inference_mode():
			...
			if not os.path.exists(f"samples/v2/{voice}/{letters_to_use[-1]}.wav"):
				...
				for i, value in enumerate(letters_to_use):
					...
					save_wav(wav=tts_audio, path="samples/v2/" + voice + "/" + value + ".wav", sample_rate=16000)
			...
			if not os.path.exists(f"samples/v2/{voice}/pitch_{str(pitch_adjustment)}/{letters_to_use[-1]}.wav"):
				print(f"\nblips generate pitch samples: {voice=}, {pitch_adjustment=}, {text=}")
				os.makedirs("samples/v2/" + voice + "/pitch_" + str(pitch_adjustment), exist_ok=True)
				for i, value in enumerate(letters_to_use):
					audio, _ = librosa.load("samples/v2/" + voice + "/" + value + ".wav", 16000)
					...
					output_sound.export("samples/v2/" + voice + "/pitch_" + str(pitch_adjustment) + "/" + value + ".wav", format="wav")
			word_letter_count = 0
			for i, letter in enumerate(text):
				...
				if letter == ' ':
					...
					if not os.path.isfile("samples/v2/" + voice + "/" + letter + ".wav"):
						continue
					if not os.path.isdir("samples/v2/" + voice + "/pitch_" + str(pitch_adjustment)):
						os.mkdir("samples/v2/" + voice + "/pitch_" + str(pitch_adjustment))
					
					letter_sound = AudioSegment.from_file("samples/v2/" + voice + "/pitch_" + str(pitch_adjustment) + "/" + letter + ".wav")
