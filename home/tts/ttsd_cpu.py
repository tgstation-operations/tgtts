...
def text_to_speech_blips():
	...
	if not os.path.exists('samples/' + voice):
		...
		response = requests.get(f"http://localhost:5003/generate-tts-blips", json={ 'text': text, 'voice': ogvoice }, stream=True)
...
if __name__ == "__main__":
	...
	serve(app, host="localhost", port=5221, threads=8, cleanup_interval=1, channel_timeout=10)
