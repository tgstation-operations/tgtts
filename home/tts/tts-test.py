...
def text_to_speech(text, tts):
	...
	with io.BytesIO() as wav_bytes:
		...
		with torch.inference_mode():
			...
			start_time = time.time()
			tts.tts(text=text, speaker=voice)
...
if __name__ == "__main__":
	...
	#preload the shit.
	text_to_speech("Hello World.", run_tts)
	
	run_data = {}
	run_start = time.time()
	start = time.time()
	res = text_to_speech("Hello World.", run_tts)
	run_data["Hello World"] = res
	
	start = time.time()
	res = text_to_speech("The quick brown fox jumps over the lazy dog.", run_tts)
	run_data["Quick Brown Fox"] = res
	
	start = time.time()
	res = text_to_speech("According to all known laws of aviation, there is no way a bee should be able to fly. Its wings are too small to get its fat little body off the ground. The bee, of course, flies anyway because bees don't care what humans think is impossible.", run_tts)
	run_data["Bee Movie Intro"] = res
	
	start = time.time()
	res = {}
	res = text_to_speech("Did you ever hear the tragedy of Darth Plagueis \"the wise\"? I thought not. It's not a story the Jedi would tell you. It's a Sith legend. Darth Plagueis was a Dark Lord of the Sith, so powerful and so wise he could use the Force to influence the midichlorians to create life... He had such a knowledge of the dark side that he could even keep the ones he cared about from dying. The dark side of the Force is a pathway to many abilities some consider to be unnatural. He became so powerful... the only thing he was afraid of was losing his power, which eventually, of course, he did. Unfortunately, he taught his apprentice everything he knew, then his apprentice killed him in his sleep. It's ironic he could save others from death, but not himself.", run_tts)
	run_data["Darth Plagueis"] = res
	
	run_data["Run total"] = time.time() - run_start
	timing_data["Old Model1"] = run_data
	...
	run_data["Run total"] = time.time() - run_start
	timing_data["Old Model2"] = run_data
	...
	with torch.inference_mode():
		run_tts = TTS("tts_models/en/vctk/vits", progress_bar=False, gpu=USE_GPU)
		#run_tts = TTS(model_path="./tts_data/model_no_disc.pth", config_path="./tts_data/config.json", progress_bar=False, gpu=USE_GPU)
	...
	run_data["Run total"] = time.time() - run_start
	timing_data["New Model1"] = run_data
	...
	run_data["Run total"] = time.time() - run_start
	timing_data["New Model2"] = run_data
