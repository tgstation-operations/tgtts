...
from julius.resample import ResampleFrac #ADDED BY MSO
resamplefunc = ResampleFrac(22050, 16000) #ADDED BY MSO
        ...
        if not reference_wav:  # not voice conversion
            for sen in sens:
                ...
                if not use_gl:
                    ...
                    #waveform = resamplefunc(waveform.cpu()) #ADDED BY MSO
                #else:
                    #waveform = resamplefunc(outputs["outputs"]["model_outputs"][0].data.float().cpu())  #ADDED BY MSO
                    #outputs["outputs"]["model_outputs"][0].data.float().cpu()
                ...
                #if wavs == None:
                #    wavs = torch.as_tensor(waveform)
                #else:
                #    torch.cat((wavs, torch.as_tensor(waveform)))
                #torch.cat((wavs, torch.as_tensor([0] * 10000)))
                wavs += list(waveform)
                wavs += [0] * 10000
        else:
